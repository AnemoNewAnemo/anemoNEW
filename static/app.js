...
let tg = null;
let webappVersion = 0;
let homeScreenStatus = "unknown"; // unsupported | unknown | added | missed

(function initTelegram() {
    const wa = window.Telegram?.WebApp;

    if (!wa) {
        console.warn("Запущено в браузере — Telegram API отключён ❗");
        return;
    }

    tg = wa;
    webappVersion = parseFloat(tg.version || "0");

    try {
        tg.ready();       // обязательный вызов
        tg.expand?.();    // занимает всю высоту
    } catch (e) {
        console.warn("Telegram WebApp API error:", e);
    }

    // Проверка статуса "добавления на главный экран" (только в API 8.0+)
    if (webappVersion >= 8 && tg.checkHomeScreenStatus) {
        tg.checkHomeScreenStatus((status) => {
            homeScreenStatus = status;
            // делаем доступным глобально
            window.homeScreenStatus = status;
        });
    } else {
        // если не поддерживается — тоже пробрасываем
        window.homeScreenStatus = "unsupported";
    }

    // Реакция на реальное добавление
    tg.onEvent?.("homeScreenAdded", () => {
        homeScreenStatus = "added";
        window.homeScreenStatus = "added";

        // сразу убираем кнопку, если она есть
...
export function requestAddToHome() {
...
const App = {
    ...
    async init() {
        ...
        // Иногда статус прилетает сразу, иногда с задержкой, поэтому две проверки
        const updateAddHomeButtonVisibility = () => {
            if (!addHomeBtn) return;

            // показываем только если Telegram сказал "missed"
            if (window.homeScreenStatus === "missed") {
        ...
        updateAddHomeButtonVisibility();

        // вторая попытка — через 300 мс, если статус придёт чуть позже
        setTimeout(updateAddHomeButtonVisibility, 300);
    ...
    addToHomeScreen() {
        ...
        if (tg?.addToHomeScreen) {
            ...
            // На всякий случай — если вдруг нет поддержки
            Notify.show("Добавление на главный экран не поддерживается", "error");
            return;
        }

        // кнопку можно сразу спрятать
        const btn = document.getElementById("btn-add-home");
        if (btn) {
            btn.style.display = "none";
        }
